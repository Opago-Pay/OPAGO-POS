...
namespace {
	...
	void handleOverSizedLogFile() {
		if (!opago_spiffs::isInitialized()) return;
		...
		if (file) {
			...
			if (logFileSize >= logFileMaxBytes) {
				for (int num = maxLogFileNum; num >= 0; num--) {
					...
					if (opago_spiffs::fileExists(oldFilePath.c_str())) {
						if (num >= maxLogFileNum) {
							opago_spiffs::deleteFile(oldFilePath.c_str());
						} else {
							const std::string newLogFilePath = logger::getLogFilePath(num + 1);
							opago_spiffs::renameFile(oldFilePath.c_str(), newLogFilePath.c_str());
						}
					}
				}
			}
