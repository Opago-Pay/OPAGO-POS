...
namespace {
	...
	void onMessage(const std::string &message) {
		...
		try {
			...
			if (method == "restart") {
				...
				docInfo["spiffsInitialized"] = opago_spiffs::isInitialized();
				...
				for (int num = 3; num >= 0; num--) {
					...
					if (opago_spiffs::fileExists(logFilePath.c_str())) {
				...
				for (int num = 3; num >= 0; num--) {
					const std::string logFilePath = logger::getLogFilePath(num);
					if (opago_spiffs::fileExists(logFilePath.c_str())) {
						opago_spiffs::deleteFile(logFilePath.c_str());
					}
				}
	...
	void parseSerialInput() {
		if (Serial.available() > 0) {
			Serial.println("Reading JSON-RPC message...");
